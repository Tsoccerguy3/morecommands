...
public class PacketHandlerClient {
	...
	public void handleXrayAction(XrayAction action) {
		switch (action) {
			case TOGGLE: 
				...
				(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage(Xray.getXray().isEnabled() ? "command.xray.enabled" : "command.xray.disabled");
				break;
			case SHOW_CONF: 
			...
			case RESET: 
				Xray.getXray().reset();
				(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage("command.xray.reseted");
				break;
			default: break;
		}
	}
	...
	public void handleXrayChangeSettings(boolean xrayEnabled) {
		...
		(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage(xrayEnabled ? "command.xray.enabled" : "command.xray.disabled");
	}
	...
	public void handleXrayChangeSettings(int blockRadius) {
		...
		(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage("command.xray.radiusSet", blockRadius);
	}
		...
		switch (action) {
			...
			case LOAD_COMBINED:
				if (settings.xray.containsKey(preset)) {
					...
					(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage("command.xray.loaded", preset);
				}
				else (new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage("command.xray.notFound", TextFormatting.RED, preset);
			...
			case SAVE:
				...
				(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage("command.xray.saved", preset);
				
				break;
			case DELETE:
				settings.xray.remove(preset);
				(new CommandSender(Minecraft.getMinecraft().player)).sendLangfileMessage("command.xray.deleted", preset);
				break;
			default:
				break;
		}
