...
public final class EntityUtils {
	...
	public static Class<? extends Entity> getEntityClass(ResourceLocation name, boolean ignoreCase) {
		...
		if (!ignoreCase) {
			...
			for (EntityEntry entry : ForgeRegistries.ENTITIES)
				if (name.getNamespace().equalsIgnoreCase(entry.getRegistryName().getNamespace()) &&
					name.getPath().equalsIgnoreCase(entry.getRegistryName().getPath())) 
	...
	public static RayTraceResult rayTraceBlock(Entity entity, double distance, float partialTickTime) {
		...
		Vec3d lookVec = entity.getLook(partialTickTime);
		Vec3d hitVec = positionVec.add(lookVec.x * distance, lookVec.y * distance, lookVec.z * distance);
