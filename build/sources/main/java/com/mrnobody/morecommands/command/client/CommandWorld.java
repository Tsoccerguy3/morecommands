		...
		if (params.length > 0) {
			if (params[0].equalsIgnoreCase("load") && params.length > 1) {
				...
				if (params[0].equalsIgnoreCase("backup")) {
					...
					copyDirectory(new File(Minecraft.getMinecraft().gameDir, "saves/" + sender.getServer().getFolderName()), new File(Minecraft.getMinecraft().gameDir, "backup/" + sender.getServer().getFolderName() + "/" + time), l);
				}
			}
				...
				File parent = new File(Minecraft.getMinecraft().gameDir, "saves");
